/**
 * Created by yuys on 2018/12/22.
 */
cBoard.controller('interfaceDataSetCtrl', function ($scope, $http, $state, $stateParams, interfacedataService, $uibModal, ModalUtils, $filter, chartService, $timeout, uuid4) {
    ...
    var originalData = [];
    var updateUrl = "interface/updateDataset.do";
    ...
    $http.get("interface/getDatasourceList.do").success(function (response) {
        $scope.datasourceList = response;
    });

    var getDatasetList = function () {
        $http.get("interface/getDatasetList.do").success(function (response) {
    ...
    var getCategoryList = function () {
        $http.get("interface/getDatasetCategoryList.do").success(function (response) {
    ...
    $scope.editDs = function (ds) {
        $http.post("interface/checkDatasource.do", {id: ds.data.datasource}).success(function (response) {
    ...
    $scope.deleteDs = function (ds) {
        $http.get("interface/getAllWidgetList.do").then(function (response) {
            if (!response) {
                return false;
            }
            var resDs = [];
            ...
            ModalUtils.confirm(translate("COMMON.CONFIRM_DELETE"), "modal-warning", "lg", function () {
                $http.post("interface/deleteDataset.do", {id: ds.id}).success(function (serviceStatus) {
    ...
    $scope.copyDs = function (ds) {
        ...
        $http.post("interface/saveNewDataset.do", {json: angular.toJson(data)}).success(function (serviceStatus) {
            if (serviceStatus.status == '1') {
                $scope.optFlag = 'none';
                getDatasetList();
    ...
    $scope.save = function () {
        ...
        if ($scope.optFlag == 'new') {
            $http.post("interface/saveNewDataset.do", {json: angular.toJson(ds)}).success(function (serviceStatus) {
                if (serviceStatus.status == '1') {
                    getCategoryList();
                    getDatasetList();
                    $scope.verify = {dsName: true};
    ...
    $scope.editFilterGroup = function (col) {
        ...
        $uibModal.open({
            templateUrl: 'org/taiji/view/config/modal/filterGroup.html',
            ...
            controller: function ($scope, $uibModalInstance) {
                ...
                $scope.editFilter = function (filter) {
                    $uibModal.open({
                        templateUrl: 'org/taiji/view/interface/modal/param.html',
                        windowTemplateUrl: 'org/taiji/view/util/modal/window.html',
                        backdrop: false,
                        size: 'lg',
                        resolve: {
                            ...
                            getSelects: function () {
                                return function (byFilter, column, callback) {
                                    interfacedataService.getDimensionValues($scope.datasource.id, $scope.curWidget.query, undefined, column, undefined, function (filtered) {
                                        callback(filtered);
                                    });
                                };
                            },
    ...
    $scope.editExp = function (col) {
        ...
        $uibModal.open({
            templateUrl: 'org/taiji/view/config/modal/exp.html',
            windowTemplateUrl: 'org/taiji/view/util/modal/window.html',
            backdrop: false,
            size: 'lg',
            scope: $scope,
    ...
    $scope.custom = function (o) {
        ...
        $uibModal.open({
            templateUrl: 'org/taiji/view/config/modal/custom.html',
            windowTemplateUrl: 'org/taiji/view/util/modal/window.html',
            backdrop: false,
            size: 'lg',
            controller: function ($scope, $uibModalInstance) {
    ...
    $scope.loadData = function (reload) {
        ...
        interfacedataService.getColumns({
    ...
    $scope.doConfigParams = function () {
        $http.get('interface/getConfigParams.do', {
            params: {
                //type: $scope.datasource.type,
                datasourceId: $scope.datasource.id,
                page: 'interfaceDataSet.html'
            }
