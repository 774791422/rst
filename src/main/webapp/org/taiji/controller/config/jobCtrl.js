...
cBoard.controller('jobCtrl', function ($scope, $rootScope, $http, dataService, $uibModal, ModalUtils, $filter, $interval) {
    ...
    $scope.jobTypes = [{name: '邮件发送', type: 'mail'},{name: '本地方法', type: 'method'}];
    ...
    $scope.runJob = function (job,state) {
        $http.post("dashboard/execJob.do", {id: job.id,state:state}).success(function (serviceStatus) {
    ...
    $scope.editJob = function (job) {
        $uibModal.open({
            templateUrl: 'org/taiji/view/config/modal/job/edit.html',
            ...
            controller: function ($scope, $uibModalInstance) {
                ...
                $scope.config = function () {
                    $uibModal.open({
                        templateUrl: 'org/taiji/view/config/modal/job/' + $scope.job.jobType + '.html',
                        windowTemplateUrl: 'org/taiji/view/util/modal/window.html',
                        backdrop: false,
                        size: 'lg',
                        scope: $scope,
